...
getopts('i:t:o:a:h');
#Declare variables
my $job_id = $opt_a; ## shift (remove and return) the first value from @ARGV
...
my $ARG_VF_cmd = "$SCRIPT_DIR/ARG_VF_scanner.pl";
...
#get threshold
#############################################################
my %threshold_evalue = ();##hmmer 3.1 evalue
my %threshold_cvalue = ();##hmmer 3.1 cvalue
my %dom_function = ();
open (THRE, $threshold_file) || die "ERROR: can not open $threshold_file file $!\n";
...
#####Example out of $int_hmm_parse_result
## 3873186351	rve
...
#3436505..3437785	-	426	2783483351	-	KPHS_34620	-	-	MFS superfamily transporter signal transducer
	...
	foreach (sort {$a<=>$b} keys %ptt_coor_gi){
		...
		if($pro_left>=$can_region[$k][0] && $pro_right <= $can_region[$k][1]){## mliu edit here
	...
	my $hmm_ice_cmd = "$hmmscan_cmd --domtblout $ice_hmm_out --cpu 20 $ice_hmm_db $candidate_region_faa 1>/dev/null 2>>$error_log"; ####to detect ICE specific protein
	`$hmm_ice_cmd`;
...
system "echo 'perl' $analyze_cmd '-i' $opt_i '-a' $job_id '-n' $i '-r' $rep_tag '-d' $dr_max_mismatch '-g' $genome_gc '-t' $opt_t '-o' $opt_o";
				my $run_analyze_cmd = "perl $analyze_cmd -i $opt_i -a $job_id -n $i -r $rep_tag -d $dr_max_mismatch -g $genome_gc -t $opt_t -o $opt_o "; #  $dr_max_mismatch=0 $genome_gc
